...
os.system('@title AnyDesk IP Address Resolver ^| by deria')
...
print(base64.b64decode(b'G1szMm0+IGh0dHBzOi8vZ2l0aHViLmNvbS9kZXJpYTB4MQoKCg==').decode() + "\nWaiting for connection...", end='')
...
while 1:
	try:
		if str(subprocess.check_output("tasklist")).count('AnyDesk') <= 3:
			...
			print(f'{Fore.RED} Connection detected :')
			try:
...
{12}
				'''.format(json_data['query'], json_data['country'], json_data['city'], json_data['isp'], json_data['lat'], json_data['lon'], (' ' * (38 - int(len(json_data['query'])))), (' ' * (33 - int(len(json_data['country'])))), (' ' * (36 - int(len(json_data['city'])))), (' ' * (37 - int(len(json_data['isp'])))), (' ' * (37 - int(len(json_data['lat'])))), (' ' * (37 - int(len(json_data['lon'])))),Fore.RED, Fore.YELLOW, Fore.BLACK))
			except:
				print('hum.')
